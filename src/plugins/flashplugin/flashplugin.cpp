...
FlashPlugin::FlashPlugin()
{
	...
	exportFileDialog_compression = NULL;
}

QString FlashPlugin::name() const
{
	return QString("Flash/SWF");
}

bool FlashPlugin::exportFile(Object* object, int startFrame, int endFrame, QMatrix view, Layer* layer, QSize exportSize, QString filePath, int fps) {
	if (!exportFileDialog) {
		createExportFileDialog();
		exportFileDialog->exec();
		if(exportFileDialog->result() == QDialog::Rejected) return false;
	} else { return false; }
	
	QSettings settings("Pencil","Pencil");
	settings.setValue("flashCompressionLevel", 10-exportFileDialog_compression->value() );
	int compression = settings.value("flashCompressionLevel").toInt();
	writeFile(object, startFrame, endFrame, view, exportSize, filePath, fps, compression);
}

void FlashPlugin::createExportFileDialog() {
	exportFileDialog = new QDialog(NULL, Qt::Dialog);
	QGridLayout *mainLayout = new QGridLayout;
	
	QSettings settings("Pencil","Pencil");
	
	
	exportFileDialog_compression = new QSlider(Qt::Horizontal);
	exportFileDialog_compression->setTickPosition(QSlider::TicksBelow);
	exportFileDialog_compression->setMinimum(0);
	exportFileDialog_compression->setMaximum(10);
	exportFileDialog_compression->setValue( 10 - settings.value("flashCompressionLevel").toInt() );
	...
	compressionLayout->addWidget(exportFileDialog_compression,0,1);
	compressionLayout->addWidget(label2,0,2);
	compressionBox->setLayout(compressionLayout);
	
	QDialogButtonBox *buttonBox = new QDialogButtonBox(QDialogButtonBox::Cancel | QDialogButtonBox::Ok);
	connect(buttonBox, SIGNAL(accepted()), exportFileDialog, SLOT(accept()));
	connect(buttonBox, SIGNAL(rejected()), exportFileDialog, SLOT(reject()));
	
	mainLayout->addWidget(compressionBox, 0, 0);
	mainLayout->addWidget(buttonBox, 1, 0);
	exportFileDialog->setLayout(mainLayout);
	exportFileDialog->setWindowTitle(tr("Options"));
	exportFileDialog->setModal(true);
}

bool FlashPlugin::writeFile(Object* object, int startFrame, int endFrame, QMatrix view, QSize exportSize, QString filePath, int fps, int compression) const {
			...
			if(layer->type == Layer::BITMAP) {
				...
				for(int frameNumber = startFrame; frameNumber <= endFrame; frameNumber++) {
					...
					if(bitmapImage != NULL) {
						...
						convertToSWFSprite( bitmapImage, view, imageSprite);
...
void FlashPlugin::convertToSWFSprite( BitmapImage* bitmapImage, QMatrix view, SWFSprite* sprite ) const {
...
void FlashPlugin::convertToSWFSprite( VectorImage* vectorImage, Object* object, QMatrix view, SWFSprite* sprite ) const {
...
void FlashPlugin::addShape( SWFSprite* sprite, QPainterPath path, QColor fillColour, QColor borderColour, qreal width, bool fill ) const {
	SWFShape* shape = new SWFShape();
...
Q_EXPORT_PLUGIN2(pencil_flashplugin, FlashPlugin)
